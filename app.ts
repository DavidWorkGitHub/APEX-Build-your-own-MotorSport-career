...
interface Trait {
  ...
  full: string;
  blurb: string;
  /** position on the circuit map, in viewBox units */
  x: number;
  y: number;
}
...
const MODES: Array<{ id: Mode; name: string; cadence: string; blurb: string; sector: string }> = [
  {
    id: "intense",
    name: "Intense",
...
const RETIRED_NUMBERS: Record<number, string> = { 17: "retired" };
...
function show(screen: "mode" | "identity" | "card"): void {
...
function paintHelmet(): void {
  ...
  document.querySelectorAll<SVGElement>(".helmet-accent").forEach((el) => el.setAttribute("fill", accent));
...
function numberError(n: number): string | null {
  if (!Number.isInteger(n) || n < 2 || n > 99) return "Pick a number between 2 and 99. Number 1 belongs to the reigning champion.";
  if (RETIRED_NUMBERS[n]) return `${n} is retired from the grid. Choose another.`;
  return null;
}
...
function validate(): boolean {
  ...
  if (!driver.trait) missing.push("racing trait");
...
function init(): void {
  ...
  $<HTMLInputElement>("#number").addEventListener("input", (e) => {
    driver.number = parseInt((e.target as HTMLInputElement).value, 10);
    if (!Number.isNaN(driver.number)) paintHelmet();
    validate();
  });
